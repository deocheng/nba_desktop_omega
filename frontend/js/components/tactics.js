...
(function () {
  ...
  // 播放状态（模块级，render 时重置）
  var state = { frames: [], meta: {}, cur: 0, playing: false, speed: 1, last: null };
  ...
  function render(containerId, mode) {
    ...
    state = { frames: [], meta: {}, cur: 0, playing: false, speed: 1, last: null };
    playerEls = {};
    tickCbs = [];

    container.innerHTML = buildLayout(mode);
    buildCourt();
    bindControls();
  ...
  function buildLayout(mode) {
    if (mode === 'clutch') return buildClutchLayout();
    return (
      ...
      '<div class="tactics-layout">' +
        ...
        '<div class="tactics-court-wrap card">' +
          '<svg id="tacticsCourt" viewBox="0 0 500 470" preserveAspectRatio="xMidYMid meet"></svg>' +
        '</div>' +
        // 右：图例
  ...
  function setFrames(frames, meta) {
    state.frames = frames || [];
    state.meta = meta || {};
    state.cur = 0;
    state.playing = false;
