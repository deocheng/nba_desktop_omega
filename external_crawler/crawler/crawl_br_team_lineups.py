...
落库: team_lineups（每 team×season×season_type×lineup 一行，5 人组合）
...
lineup_key 为 5 个 br_player_id 排序后 '|' 连接（无序去重）。

仅抓取 BR 实际列出的 5 人组合（与 starting_lineups「存实际组合」先例一致）；
...
def parse_team_lineups_html(html: str, season_type: str = "Regular",
    ...
    每条记录含 ``players``（(br_slug, name) 列表，长度须为 5）与全部数值列；
    不含 team/season 维度（由 ``build_rows`` 注入）。BR 按阵容人数拆成多张表
    （2/3/4/5-man），我们只取「整队 5 人组合」表：常规赛 id=``lineups_5-man_``、
    季后赛 id=``lineups_5-man__p``。
    """
    if not html:
        return []
    soup = BeautifulSoup(html, "html.parser")
    tid = "lineups_5-man__p" if season_type == "Playoffs" else "lineups_5-man_"
